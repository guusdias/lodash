(function () {
  ...
  const noop = function () {};
  let params = root.arguments;
  const { system } = root;

  /** Add `console.log()` support for Rhino and RingoJS. */
  const console = root.console || (root.console = { log: root.print });
  ...
  const buildName = root.buildName = basename(ui.buildPath, '.js');

  /** The other library basename. */
  const otherName = root.otherName = (function () {
    const result = basename(ui.otherPath, '.js');
  ...
  const load = (typeof require === 'function' && !amd)
    ? require
    : noop;
  ...
  function basename(filePath, extension) {
    const result = (filePath || '').split(rePathSeparator).pop();
  ...
  function getGeometricMean(array) {
    return Math.E ** (lodash.reduce(array, (sum, x) => sum + Math.log(x), 0) / array.length) || 0;
  }
