...
(function () {
  ...
  function basePick(object, props) {
    object = Object(object);
    return reduce(props, (result, key) => {
      if (key in object) {
  ...
  function baseWrapperValue(value, actions) {
    const result = value;
    return reduce(actions, (result, action) => action.func.apply(action.thisArg, arrayPush([result], action.args)), result);
  }
  ...
  function equalArrays(array, other, bitmask, customizer, equalFunc, stack) {
    ...
    while (++index < arrLength) {
      ...
      if (seen) {
        if (!baseSome(other, (othValue, othIndex) => {
   ...
   * @category Collection
   * @param {Array | object} collection The collection to iterate over.
   * @param {...(Function|Function[])} [iteratees]
  ...
  function sortBy(collection, iteratee) {
    ...
    return baseMap(baseMap(collection, (value, key, collection) => ({ value, index: index++, criteria: iteratee(value, key, collection) })).sort((object, other) => compareAscending(object.criteria, other.criteria) || (object.index - other.index)), baseProperty('value'));
  }
   ...
   * // => 'hi fred!'
   */
  const bind = baseRest((func, thisArg, partials) => createPartial(func, WRAP_BIND_FLAG | WRAP_PARTIAL_FLAG, thisArg, partials));
  ...
  baseEach(['pop', 'join', 'replace', 'reverse', 'split', 'push', 'shift', 'sort', 'splice', 'unshift'], methodName => {
    const func = (/^(?:replace|split)$/.test(methodName) ? String.prototype : arrayProto)[methodName];
