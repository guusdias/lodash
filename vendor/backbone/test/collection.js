(function (QUnit) {
  let a; let b; let c; let d; let e; let col; let
    otherCol;
  ...
  QUnit.test('new and parse', assert => {
    ...
    const Collection = Backbone.Collection.extend({
      parse(data) {
        return _.filter(data, datum => datum.a % 2 === 0);
      },
    });
  ...
  QUnit.test('Underscore methods', assert => {
    ...
    assert.deepEqual(col.countBy(model => model.id), {
      0: 1, 1: 1, 2: 1, 3: 1,
    });
  ...
  QUnit.test('reset', assert => {
    ...
    col.reset(_.map(models, m => m.attributes));
  ...
  QUnit.test('#1655 - sortBy can be used with a string argument.', assert => {
    ...
    const values = _.map(collection.sortBy('x'), model => model.get('x'));
