(function (QUnit) {
  ...
  QUnit.test('chain', assert => {
    ...
    assert.deepEqual(model.chain().pick('a', 'b', 'c').values().compact()
      .value(), [1, 2]);
  });
  ...
  QUnit.test('matches with predicate', assert => {
    ...
    assert.strictEqual(model.matches(attr => attr.a > 1 && attr.b != null), false);

    model.set({ a: 3, b: true });

    assert.strictEqual(model.matches(attr => attr.a > 1 && attr.b != null), true);
  });

  QUnit.test('set and unset', assert => {
    assert.expect(8);
    const a = new Backbone.Model({
      id: 'id', foo: 1, bar: 2, baz: 3,
    });
    let changeCount = 0;
