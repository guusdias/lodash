(function () {
  ...
  QUnit.test('allKeys', function (assert) {
    ...
    _.each([null, void 0, 1, 'a', true, NaN, {}, [], new Number(5), new Date(0)], val => {
  ...
  QUnit.test('invert', assert => {
    let obj = { first: 'Moe', second: 'Larry', third: 'Curly' };
  ...
  QUnit.test('functions', assert => {
    const obj = {
      a: 'dash', b: _.map, c: /yo/, d: _.reduce,
    };
  ...
  QUnit.test('extendOwn', assert => {
    ...
    result = _.extendOwn({
      a: 1, 0: 2, 1: '5', length: 6,
    }, { 0: 1, 1: 2, length: 2 });
    assert.deepEqual(result, {
      a: 1, 0: 1, 1: 2, length: 2,
    }, 'should treat array-like objects like normal objects');
  });

  QUnit.test('assign', assert => {
    assert.strictEqual(_.assign, _.extendOwn, 'is an alias for extendOwn');
  });

  QUnit.test('pick', assert => {
    let result;
    ...
    const callback = function (value, key, object) {
  ...
  QUnit.test('defaults', assert => {
    let options = {
      zero: 0, one: 1, empty: '', nan: NaN, nothing: null,
    };

    _.defaults(options, {
      zero: 1, one: 10, twenty: 20, nothing: 'str',
    });
  ...
  QUnit.test('clone', assert => {
    const moe = { name: 'moe', lucky: [13, 27, 34] };
  ...
  QUnit.test('create', assert => {
    ...
    const func = function () {};
    Child.prototype = _.create(Parent.prototype, { func });
    ...
    const created = _.create(Child.prototype, new Child());
  ...
  QUnit.test('isEqual', assert => {
    ...
    assert.notOk(_.isEqual(new String('Curly'), { toString() { return 'Curly'; } }), 'String objects and objects with a custom `toString` method are not equal');
    ...
    assert.notOk(_.isEqual(new Number(63), { valueOf() { return 63; } }), 'Number objects and objects with a `valueOf` method are not equal');
  ...
  QUnit.test('isObject', function (assert) {
    ...
    assert.ok(_.isObject(() => {}), 'and functions');
  ...
  QUnit.test('isFunction', assert => {
    ...
    assert.ok(_.isFunction(() => {}), 'even anonymous ones');
  ...
  if (typeof Int8Array !== 'undefined') {
    QUnit.test('#1929 Typed Array constructors are functions', assert => {
  ...
  QUnit.test('tap', assert => {
    let intercepted = null;
    const interceptor = function (obj) { intercepted = obj; };
    let returned = _.tap(1, interceptor);
  ...
  QUnit.test('isMatch', assert => {
    ...
    _.each([null, void 0], item => { assert.strictEqual(_.isMatch(item, null), true, 'null matches null'); });
    _.each([null, void 0], item => { assert.strictEqual(_.isMatch(item, null), true, 'null matches {}'); });
    assert.strictEqual(_.isMatch({ b: 1 }, { a: void 0 }), false, 'handles undefined values (1683)');

    _.each([true, 5, NaN, null, void 0], item => {
  ...
  QUnit.test('matcher', assert => {
    ...
    Prototest.prototype.x = 1;
    const specObj = new Prototest();
    let protospec = _.matcher(specObj);
    ...
    // null edge cases
    const oCon = _.matcher({ constructor: Object });
  ...
  QUnit.test('findKey', assert => {
    ...
    assert.equal(_.findKey(objects, obj => obj.a === 0), 'a');

    assert.equal(_.findKey(objects, obj => obj.b * obj.a === 4), 'c');

    assert.equal(_.findKey(objects, 'a'), 'b', 'Uses lookupIterator');

    assert.equal(_.findKey(objects, obj => obj.b * obj.a === 5), void 0);

    assert.strictEqual(_.findKey([1, 2, 3, 4, 5, 6], obj => obj === 3), '2', 'Keys are strings');

    assert.strictEqual(_.findKey(objects, a => a.foo === null), void 0);
    ...
    const array = [1, 2, 3, 4];
    array.match = 55;
    assert.strictEqual(_.findKey(array, x => x === 55), 'match', 'matches array-likes keys');
  });
  ...
  QUnit.test('mapObject', assert => {
    ...
    assert.deepEqual(_.mapObject(obj, val => val * 2), { a: 2, b: 4 }, 'simple objects');

    assert.deepEqual(_.mapObject(objects, val => _.reduce(val, (memo, v) => memo + v, 0)), { a: 0, b: 2, c: 4 }, 'nested objects');

    assert.deepEqual(_.mapObject(obj, (val, key, o) => o[key] * 2), { a: 2, b: 4 }, 'correct keys');

    assert.deepEqual(_.mapObject([1, 2], val => val * 2), { 0: 2, 1: 4 }, 'check behavior for arrays');
