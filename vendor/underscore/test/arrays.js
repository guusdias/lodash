(function () {
  ...
  QUnit.test('first', assert => {
    ...
    let result = (function () { return _.first(arguments); }(4, 3, 2, 1));
  ...
  QUnit.test('rest', assert => {
    ...
    let result = (function () { return _(arguments).rest(); }(1, 2, 3, 4));
  ...
  QUnit.test('initial', assert => {
    ...
    let result = (function () { return _(arguments).initial(); }(1, 2, 3, 4));
  ...
  QUnit.test('compact', assert => {
    assert.deepEqual(_.compact([1, false, null, 0, '', void 0, NaN, 2]), [1, 2], 'removes all falsy values');
    let result = (function () { return _.compact(arguments); }(0, 1, false, 2, false, 3));
    assert.deepEqual(result, [1, 2, 3], 'works on an arguments object');
  ...
  QUnit.test('flatten', assert => {
    ...
    const result = (function () { return _.flatten(arguments); }(1, [2], [3, [[[4]]]]));
  ...
  QUnit.test('uniq', assert => {
    ...
    let expected = [{ name: 'Moe' }, { name: 'Curly' }, { name: 'Larry' }];
    let iterator = function (stooge) { return stooge.name; };
    ...
    const result = (function () { return _.uniq(arguments); }(1, 2, 1, 3, 1, 4));
    assert.deepEqual(result, [1, 2, 3, 4], 'works on an arguments object');
  ...
  QUnit.test('intersection', assert => {
    const stooges = ['moe', 'curly', 'larry']; const
      leaders = ['moe', 'groucho'];
    ...
    let result = (function () { return _.intersection(arguments, leaders); }('moe', 'curly', 'larry'));
    assert.deepEqual(result, ['moe'], 'works on an arguments object');
    const theSixStooges = ['moe', 'moe', 'curly', 'curly', 'larry', 'larry'];
  ...
  QUnit.test('union', assert => {
    let result = _.union([1, 2, 3], [2, 30, 1], [1, 40]);
  ...
  QUnit.test('difference', assert => {
    let result = _.difference([1, 2, 3], [2, 30, 40]);
  ...
  QUnit.test('zip', assert => {
    const names = ['moe', 'larry', 'curly']; const ages = [30, 40, 50]; const
      leaders = [true];
    ...
    let stooges = _.zip(['moe', 30, 'stooge 1'], ['larry', 40, 'stooge 2'], ['curly', 50, 'stooge 3']);
  ...
  QUnit.test('object', assert => {
    let result = _.object(['moe', 'larry', 'curly'], [30, 40, 50]);
    let shouldBe = { moe: 30, larry: 40, curly: 50 };
  ...
  QUnit.test('indexOf', assert => {
    ...
    const result = (function () { return _.indexOf(arguments, 2); }(1, 2, 3));
    assert.equal(result, 1, 'works on an arguments object');

    _.each([null, void 0, [], false], val => {
      const msg = `Handles: ${_.isArray(val) ? '[]' : val}`;
    ...
    let index = _.indexOf(numbers, num, true);
    ...
    assert.ok(_.every(['1', [], {}, null], () => _.indexOf(numbers, num, {}) === 1), 'non-nums as fromIndex make indexOf assume sorted');
  ...
  QUnit.test('indexOf with NaN', assert => {
  ...
  QUnit.test('indexOf with +- 0', assert => {
    _.each([-0, +0], val => {
  ...
  QUnit.test('lastIndexOf', assert => {
    let numbers = [1, 0, 1];
    const falsey = [void 0, '', 0, false, NaN, null, void 0];
    ...
    const result = (function () { return _.lastIndexOf(arguments, 1); }(1, 0, 1, 0, 0, 1, 0, 0, 0));
    ...
    numbers = [1, 2, 3, 1, 2, 3, 1, 2, 3];
    const index = _.lastIndexOf(numbers, 2, 2);
    ...
    _.each([6, 8, 2 ** 32, Infinity], fromIndex => {
    ...
    const expected = _.map(falsey, value => (typeof value === 'number' ? -1 : 5));

    const actual = _.map(falsey, fromIndex => _.lastIndexOf(array, 3, fromIndex));
    ...
    assert.deepEqual(_.map([-6, -8, -Infinity], fromIndex => _.lastIndexOf(array, 1, fromIndex)), [0, -1, -1]);
  });

  QUnit.test('lastIndexOf with NaN', assert => {
  ...
  QUnit.test('lastIndexOf with +- 0', assert => {
    _.each([-0, +0], val => {
  ...
  QUnit.test('findIndex', assert => {
    ...
    assert.equal(_.findIndex(objects, obj => obj.a === 0), 0);

    assert.equal(_.findIndex(objects, obj => obj.b * obj.a === 4), 2);

    assert.equal(_.findIndex(objects, 'a'), 1, 'Uses lookupIterator');

    assert.equal(_.findIndex(objects, obj => obj.b * obj.a === 5), -1);

    assert.equal(_.findIndex(null, _.noop), -1);
    assert.strictEqual(_.findIndex(objects, a => a.foo === null), -1);
    ...
    const sparse = [];
    sparse[20] = { a: 2, b: 2 };
    assert.equal(_.findIndex(sparse, obj => obj && obj.b * obj.a === 4), 20, 'Works with sparse arrays');

    const array = [1, 2, 3, 4];
    array.match = 55;
    assert.strictEqual(_.findIndex(array, x => x === 55), -1, 'doesn\'t match array-likes keys');
  });

  QUnit.test('findLastIndex', assert => {
    const objects = [
      { a: 0, b: 0 },
      { a: 1, b: 1 },
      { a: 2, b: 2 },
    ...
    assert.equal(_.findLastIndex(objects, obj => obj.a === 0), 3);

    assert.equal(_.findLastIndex(objects, obj => obj.b * obj.a === 4), 2);

    assert.equal(_.findLastIndex(objects, 'a'), 2, 'Uses lookupIterator');

    assert.equal(_.findLastIndex(objects, obj => obj.b * obj.a === 5), -1);

    assert.equal(_.findLastIndex(null, _.noop), -1);
    assert.strictEqual(_.findLastIndex(objects, a => a.foo === null), -1);
    ...
    const sparse = [];
    sparse[20] = { a: 2, b: 2 };
    assert.equal(_.findLastIndex(sparse, obj => obj && obj.b * obj.a === 4), 20, 'Works with sparse arrays');

    const array = [1, 2, 3, 4];
    array.match = 55;
    assert.strictEqual(_.findLastIndex(array, x => x === 55), -1, 'doesn\'t match array-likes keys');
  });
