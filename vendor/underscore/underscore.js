...
(function () {
  ...
  const ArrayProto = Array.prototype; const
  ...
  _.invoke = restArgs((obj, method, args) => {
  ...
  _.sortBy = function (obj, iteratee, context) {
    let index = 0;
    iteratee = cb(iteratee, context);
    return _.pluck(_.map(obj, (value, key, list) => ({
      value,
      index: index++,
  ...
  _.partition = group((result, value, pass) => {
  ...
  _.without = restArgs((array, otherArrays) => _.difference(array, otherArrays));
  ...
  _.union = restArgs(arrays => _.uniq(flatten(arrays, true, true)));
  ...
  _.difference = restArgs((array, rest) => {
  ...
  // available.
  _.bind = restArgs((func, context, args) => {
  ...
  _.partial = restArgs((func, boundArgs) => {
    const { placeholder } = _.partial;
  ...
  _.delay = restArgs((func, wait, args) => setTimeout(() => func.apply(null, args), wait));
  ...
  _.omit = restArgs((obj, keys) => {
    let iteratee = keys[0]; let
      context;
    if (_.isFunction(iteratee)) {
